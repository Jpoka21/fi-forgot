...
const SAMPLE_CLIENTS: Client[] = [
  {
    id: "sample-1", businessId: "", fullName: "John Smith",
    relationship: "Buyer", birthday: "May 14", lastCardSent: "Christmas 2025",
    automationsOn: true, tags: "Golf, Kids",
  },
  {
    id: "sample-2", businessId: "", fullName: "Sarah Johnson",
    relationship: "Seller", birthday: "Aug 3", lastCardSent: "Birthday 2025",
    automationsOn: true, tags: "Luxury",
  },
  {
    id: "sample-3", businessId: "", fullName: "Michael Davis",
    relationship: "Past Client", birthday: "Dec 20", lastCardSent: "Holiday 2025",
    automationsOn: false, tags: "Investor",
  },
];
...
function getApiBase() {
  return `${BASE_URL}api`.replace(/\/+/g, "/").replace(/^\//, "/");
}

function getBusiness() {
  try {
    const raw = localStorage.getItem("fi_forgot_business");
    if (!raw) return null;
    return JSON.parse(raw) as { businessId: string; name: string; businessName: string; email: string; businessType?: string };
  } catch { return null; }
}
...
function AddClientPanel({ open, onClose, onSaved, businessId }: AddClientPanelProps) {
  ...
  const checkbox = (name: string, label: string, defaultChecked = true) => (
  ...
  return (
    <>
      ...
      {open && (
        <div
          onClick={onClose}
          style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.35)", zIndex: 99, transition: "opacity 0.2s" }}
        />
      )}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button
            onClick={onClose}
            style={{ background: "none", border: "none", cursor: "pointer", fontSize: "1.4rem", color: "#94a3b8", lineHeight: 1, padding: 4 }}
          >×</button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px 24px" }}>
          <form ref={formRef} onSubmit={handleSubmit} id="add-client-form">
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input
                name="fullName" required placeholder="Jane Doe" autoFocus
                style={{ width: "100%", padding: "9px 12px", border: "1.5px solid #e2e8f0", borderRadius: 7, fontSize: "0.95rem", background: "#f8fafc", boxSizing: "border-box" as const }}
              />
            </div>
            {input("birthday", "Birthday", "e.g. May 14")}
            {input("address", "Address", "123 Main St, Austin TX 78701")}
            ...
            {!expanded && (
              <button
                ...
                style={{
                  ...
                  borderRadius: 8, color: "#475569", fontSize: "0.88rem",
            ...
            {expanded && (
              <>
                ...
                {input("fullName", "Full Name *", "Jane Doe")}
                {input("company", "Company", "Smith Realty")}
                {input("email", "Email", "jane@example.com", "email")}
                {input("phone", "Phone", "(555) 555-5555", "tel")}
                {input("relationship", "Relationship", "Buyer / Seller / Past Client")}

                {sectionTitle("Important Dates")}
                {input("birthday", "Birthday", "e.g. May 14")}
                {input("homePurchaseAnniversary", "Home Purchase Anniversary", "e.g. June 2023")}
                {input("clientSince", "Client Since", "e.g. 2021")}
                ...
                <button
                  type="button" onClick={addCustomEvent}
                  style={{
                    padding: "7px 14px", marginBottom: 8,
                    background: "none", border: "1.5px dashed #cbd5e1",
                    borderRadius: 6, color: "#64748b", fontSize: "0.82rem",
                    cursor: "pointer", fontWeight: 600,
                  }}
                >+ Add Custom Event</button>

                {sectionTitle("Relationship Notes")}
                {input("kidsNames", "Kids' Names", "Emma, Liam")}
                {input("pets", "Pets", "Max (golden retriever)")}
                {input("interests", "Interests", "Golf, Travel, Wine")}
                {input("tags", "Tags", "VIP, Referral Source")}
                <div style={{ marginBottom: 14 }}>
                  ...
                  <textarea
                    name="notes" rows={3} placeholder="Anything else worth remembering…"
                ...
                {checkbox("autoBirthday", "Birthday Cards", true)}
                {checkbox("autoHoliday", "Holiday Cards", true)}
                {checkbox("autoAnniversary", "Anniversary Cards", false)}
                {checkbox("requireApproval", "Require Approval Before Sending", true)}
              </>
            )}
          </form>
        </div>
        ...
        <div style={{
          ...
          >Cancel</button>
          <button
            type="submit" form="add-client-form" disabled={saving}
            style={{
              flex: 2, padding: "11px",
              background: saving ? "#94a3b8" : RED,
...
export default function BusinessDashboardPage() {
  ...
  useEffect(() => {
    if (!business) { setLocation("/business/signup"); return; }
    fetch(`/api/business-clients?businessId=${encodeURIComponent(business.businessId)}`)
      .then(r => r.json())
      .then(data => {
  ...
  function handleClientSaved(client: Client) {
    setClients(prev => {
      const withoutSamples = prev.filter(c => !c.id.startsWith("sample-"));
      return [...withoutSamples, client];
    });
  }
  ...
  const cell: React.CSSProperties = {
    ...
    fontSize: "0.92rem", color: "#334155", verticalAlign: "middle",
  };
  ...
  const th: React.CSSProperties = {
    ...
    color: "#94a3b8", borderBottom: "2px solid #f1f5f9",
    whiteSpace: "nowrap",
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8fafc", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <input
              ...
              style={{
                ...
                fontSize: "0.88rem", outline: "none", width: 200,
              }}
            />
          </div>
      ...
      <main style={{ flex: 1, padding: "28px 28px 48px", maxWidth: 1200, width: "100%", margin: "0 auto", boxSizing: "border-box" }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={7} style={{ ...cell, textAlign: "center", color: "#94a3b8", padding: "48px 16px" }}>
                  ...
                  <tr key={c.id} style={{ transition: "background 0.1s" }}
                    ...
                    <td style={cell}>
                      <span style={{
                        ...
                        background: c.automationsOn !== false ? "#dcfce7" : "#f1f5f9",
                        color: c.automationsOn !== false ? "#16a34a" : "#94a3b8",
                      }}>
                    ...
                    <td style={{ ...cell, textAlign: "center" }}>
                      <button
                        style={{ background: "none", border: "none", cursor: "pointer", color: "#cbd5e1", fontSize: "1.1rem", padding: 4 }}
                        title="More options"
                      >⋯</button>
                    </td>
                  </tr>
        ...
        {/* Add client button in table area if empty */}
        ...
        <div style={{
          ...
          <h2 style={{ margin: "0 0 16px", fontSize: "0.92rem", fontWeight: 700, color: NAVY, letterSpacing: "0.04em", textTransform: "uppercase" }}>
            Recently Remembered
          </h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {ACTIVITY.map((a, i) => (
              <div key={i} style={{
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ margin: 0, fontSize: "0.9rem", color: "#334155" }}>{a.text}</p>
                </div>
