...
export type HypothesisAction='confirm'|'disagree'|'withdraw'|'reverse';
...
export function changeHypothesisOperation(recipientId:string,item:FiTimelineItem,action:HypothesisAction,service:Service=timelineService,operationId=crypto.randomUUID()){return runTimelineMutation(()=>service.changeHypothesis(recipientId,item.id,action,item.revision!,operationId),()=>service.getTimeline(recipientId),value=>value.items.some(current=>current.id===item.id&&current.actionHistory.some(record=>record.operationId===operationId&&record.action===action&&record.expectedRevision===item.revision)));}
