...
import { LEGACY_PLAN_ORDER, PLANS, Plan, canActivateRecipient, getCardLimit, isLegacyPlan, resolveUserPlan, type LegacyPlan } from "@/lib/plan";
import { FiRelationshipUpgradeModal } from "@/app/components/upgrade/FiRelationshipUpgradeModal";
...
function RecipientProfileHeaderIllustration() {
  return (
    <div style={{ margin: "0 0 16px", width: "100%", maxWidth: 280, display: "flex", alignItems: "center", justifyContent: "flex-start" }}>
      <img
        src={illustrationPaths.recipient.profileHeader}
...
export function FiRecipientManagementPage() {
  ...
  const atNewRecipientLimit = isNew && !canActivateRecipient(plan, activeCount);
  // Card balance: total occasions across all recipients vs. plan cap (legacy tiers only)
  ...
  const cardCap = import.meta.env.DEV ? 9999 : getCardLimit(plan);
  ...
  useEffect(() => {
    if (atNewRecipientLimit && plan === "free") {
      setUpgradeReason("recipient-limit");
      setUpgradeOpen(true);
    }
  }, [atNewRecipientLimit, plan]);
  ...
  function toggleEvent(event: string) {
    ...
    if (current.includes(event)) {
      ...
      if (Number.isFinite(cardCap) && projectedTotal > cardCap) {
  ...
  function onSubmit(data: FormData) {
    if (isNew && !canActivateRecipient(plan, activeCount)) {
      setUpgradeReason("recipient-limit");
      setUpgradeOpen(true);
      return;
    }
  ...
  return (
    <>
      <div className="fi-recipient-management">
      ...
      {/* Upgrade modal */}
      {upgradeOpen && plan === "free" && upgradeReason === "recipient-limit" ? (
        <FiRelationshipUpgradeModal onClose={() => setUpgradeOpen(false)} />
      ) : null}
      {upgradeOpen && (isLegacyPlan(plan) || upgradeReason === "card-limit") ? (
        <ProfileUpgradeModal
          currentPlan={plan as LegacyPlan}
...
function ProfileUpgradeModal({
  ...
  const orderedPlans = LEGACY_PLAN_ORDER;
  const cardCap = getCardLimit(currentPlan);

  const subtitle = reason === "card-limit"
    ? Number.isFinite(cardCap)
      ? `You've used all ${cardCap} card slots on your current plan. Upgrade to plan more occasions across your people.`
      : "Upgrade your Relationship Membership to plan more occasions across your people."
    : recipientName
