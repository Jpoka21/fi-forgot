/**
 * Governed Handoff Withdrawal — FI-DSN-STD-015 HOF-G6-U3 (R98–R111).
 *
 * Distinct HGA withdrawal act; HGA-initiated retraction / active cessation of
 * forward Handoff reliance (R53 / R105). Peer-distinct from suspension temporary
 * pause (R52) and recall responsive termination (R55 / U4).
 *
 * Does NOT recall, suspend-as-withdrawal, resume, restore, or reenter.
 * Does NOT mint a generic performHgaAct factory.
 *
 * Raw constructors — prefer Domain3Repository.withdrawGovernedHandoff.
...
export const GOVERNED_HANDOFF_WITHDRAWAL_TRACEABILITY =
  createStd015GovernanceTraceability([...HOF_G6_U3_REQUIREMENTS]);

const WITHDRAWAL_BASIS_KIND_SET = new Set<string>([
  "hga_initiated_forward_reliance_retraction_warranted",
]);

export function isWithdrawalConstitutionalBasisKind(
  value: unknown,
): value is WithdrawalConstitutionalBasisKind {
  return typeof value === "string" && WITHDRAWAL_BASIS_KIND_SET.has(value);
}

export function assertWithdrawalConstitutionalBasisKind(
  value: unknown,
): asserts value is WithdrawalConstitutionalBasisKind {
  if (!isWithdrawalConstitutionalBasisKind(value)) {
    throw new OrchestraConstitutionalError(
      "Handoff withdrawal requires a closed constitutional basisKind; free-text notes cannot be the sole basis (R99b/R103)",
      "invalid_handoff_withdrawal",
      ["FI-DSN-STD-015-R99", "FI-DSN-STD-015-R103"],
    );
  }
}

const WITHDRAWAL_FORBIDDEN_KEYS = [
  "recallActId",
  "expiryActId",
  "resumeHandoff",
  "restoreHandoff",
  ...
  "brainWithdrawHandoff",
  "brainHandoffWithdrawal",
  "brainAuthorizesHandoff",
  "implicitWithdrawal",
  "automaticInheritanceWithdrawal",
  "inferredEligibilityWithdrawal",
  "configurationDrivenWithdrawal",
  "rtcCatalogAlone",
  "hrtcmRtcAlone",
  "rejectHandoff",
  "hoemRecallRecordId",
...
export function createGovernedHandoffWithdrawalActId(): GovernedHandoffWithdrawalActId {
  return `governed-handoff-withdrawal-act-${randomUUID()}` as GovernedHandoffWithdrawalActId;
}

export function createHoemWithdrawalOperativeRecordId(): HoemWithdrawalOperativeRecordId {
  return `hoem-withdrawal-operative-${randomUUID()}` as HoemWithdrawalOperativeRecordId;
}

export function assertNoHandoffWithdrawalRecallOrReentryClaims(
  input: Record<string, unknown>,
): void {
  for (const key of WITHDRAWAL_FORBIDDEN_KEYS) {
    const value = input[key];
    if (value === true || (typeof value === "string" && value.trim()) || Array.isArray(value)) {
      throw new OrchestraConstitutionalError(
        "Handoff withdrawal must not recall, resume, restore, reenter, execute, reject, or claim RTC-alone mint (R98/R110/R111)",
        "invalid_handoff_withdrawal",
        ["FI-DSN-STD-015-R98", "FI-DSN-STD-015-R110", "FI-DSN-STD-015-R111"],
      );
    }
  }
}
...
/**
 * R70 / R98 — HGA class required; actor attribution is distinct and cannot mint authority.
 */
export function assertGovernedHandoffWithdrawalActor(input: {
  withdrawnBy: string;
  ...
  assertEstablishedHandoffGovernanceAuthorityForWithdrawal(input.authorityClassId);
  ...
  if (
    ...
    throw new OrchestraConstitutionalError(
      "Brain cannot perform Handoff withdrawal acts (R22/R70/R98)",
      "invalid_handoff_withdrawal",
      ["FI-DSN-STD-015-R22", "FI-DSN-STD-015-R70", "FI-DSN-STD-015-R98"],
    );
  }
  ...
  if (!withdrawnBy) {
    throw new OrchestraConstitutionalError(
      "Handoff withdrawal requires attributable withdrawnBy actor within HGA scope; actor string alone is not HGA authority (R98)",
      "invalid_handoff_withdrawal",
      ["FI-DSN-STD-015-R98"],
    );
  }
  const lower = withdrawnBy.toLowerCase();
  if (
    lower === "brain_runtime" ||
    ...
    throw new OrchestraConstitutionalError(
      "withdrawnBy must not mint Brain or HAAM-prohibited authority-class identity as Handoff withdrawer (R70/R98)",
      "invalid_handoff_withdrawal",
      ["FI-DSN-STD-015-R70", "FI-DSN-STD-015-R98"],
    );
  }
...
/**
 * R98–R101 / R75 — assess whether a lawful HGA withdrawal act may be performed.
 * Does NOT require G11 export_ready. Does NOT require prior suspension (R99/R100).
 * Suspension temporary pause does NOT negate prior auth/posture attributability (R99a).
 */
export function assessGovernedHandoffWithdrawal(input: {
  ...
  currentWithdrawalAlreadyCeasedReliance?: unknown;
  ...
  // R99(a): prior authorization + posture-relevant chain remain attributable even under suspension pause.
  ...
  const basisKind = isWithdrawalConstitutionalBasisKind(input.constitutionalBasisKind)
  ...
  // R100 — prior withdrawal / recall / Expired / Rejected. Suspension (R52) does NOT count.
  if (
    input.purportedWithdrawalRecordPresent === true ||
    input.purportedRecallRecordPresent === true ||
    input.currentWithdrawalAlreadyCeasedReliance === true ||
  ...
  const mayWithdraw = denialReasons.length === 0;
  return Object.freeze({
    mayWithdraw,
    denialReasons: Object.freeze([...denialReasons]),
    authorityClassId: mayWithdraw ? ("handoff_governance_authority" as const) : null,
...
/**
 * R109 — tip selection by withdrawnAt; currency evaluation is separate and subordinate.
 * After a current withdrawal, R100 blocks further operative cessation (tip remains historical).
 */
export function selectAuthoritativeGovernedHandoffWithdrawal(
  withdrawals: readonly GovernedHandoffWithdrawalActRecord[],
): GovernedHandoffWithdrawalActRecord | null {
  if (withdrawals.length === 0) return null;
  return [...withdrawals].sort((a, b) => a.withdrawnAt.localeCompare(b.withdrawnAt)).at(-1)!;
}

export function evaluateHandoffWithdrawalCurrencyFromFacts(input: {
  ...
  authoritativeWithdrawalActId: GovernedHandoffWithdrawalActId | null;
  gpraValidityPosture: GpraValidityPosture | null;
  lineageMatchesAuthoritativeGpra: boolean;
}): HandoffWithdrawalCurrency {
  if (
  ...
  if (
    input.authoritativeWithdrawalActId != null &&
    input.withdrawal.withdrawalActId !== input.authoritativeWithdrawalActId
  ) {
    return "stale";
  }
...
export interface CreateGovernedHandoffWithdrawalActInput {
  ...
  readonly brainWithdrawHandoff?: unknown;
  readonly brainHandoffWithdrawal?: unknown;
  readonly implicitWithdrawal?: unknown;
...
/**
 * Construct an operative HGA withdrawal act + HOEM withdrawal record.
 * Caller must have verified R99–R102 prerequisites.
 */
export function createGovernedHandoffWithdrawalActRecord(
  input: CreateGovernedHandoffWithdrawalActInput,
): GovernedHandoffWithdrawalActRecord {
  assertNoHandoffWithdrawalRecallOrReentryClaims(
    input as unknown as Record<string, unknown>,
  );
  ...
  assertWithdrawalConstitutionalBasisKind(input.constitutionalBasisKind);
  ...
  if (binding.entryId !== entry.entryId) {
    throw new OrchestraConstitutionalError(
      "Handoff withdrawal requires HCCM binding belonging to the provided G1 entry (R102)",
      "invalid_handoff_withdrawal",
      ["FI-DSN-STD-015-R102"],
    );
  }
  if (authorization.entryId !== entry.entryId) {
    throw new OrchestraConstitutionalError(
      "Handoff withdrawal requires authorization belonging to the provided entry (R99a/R102)",
      "invalid_handoff_withdrawal",
  ...
  if (authorization.consumerClassId !== binding.consumerClassId) {
    throw new OrchestraConstitutionalError(
      "Handoff withdrawal requires authorization matching the binding consumer class (R99a/R102)",
      "invalid_handoff_withdrawal",
      ["FI-DSN-STD-015-R99", "FI-DSN-STD-015-R102"],
    );
  }
  if (posture.bindingId !== binding.bindingId || posture.entryId !== entry.entryId) {
    throw new OrchestraConstitutionalError(
      "Handoff withdrawal requires authoritative posture belonging to the provided binding and entry (R99a/R102)",
      "invalid_handoff_withdrawal",
      ["FI-DSN-STD-015-R99", "FI-DSN-STD-015-R102"],
    );
  }
  if (
    ...
    throw new OrchestraConstitutionalError(
      "Handoff withdrawal rejected: binding lineage does not match entry (R102)",
      "invalid_handoff_withdrawal",
      ["FI-DSN-STD-015-R102"],
    );
  }
  ...
  const notes =
    ...
    input.constitutionalBasisNotes.trim()
      ? input.constitutionalBasisNotes.trim()
      : null;
  const provenance: WithdrawalConstitutionalBasisProvenance = Object.freeze({
    basisKind,
    notes,
    notesCannotBeSoleBasis: true as const,
  });

  const retractionTargets = Object.freeze([
    "authorization",
    "posture",
  ]) as readonly WithdrawalRetractionTarget[];

  const now = input.withdrawnAt ?? new Date().toISOString();
  const withdrawalActId = createGovernedHandoffWithdrawalActId();
  const hoemWithdrawalRecord: HoemWithdrawalOperativeRecord = Object.freeze({
    hoemWithdrawalRecordId: createHoemWithdrawalOperativeRecordId(),
    withdrawalActId,
  ...
  return Object.freeze({
    ...
    authorityConstitutionalScope: "handoff_withdrawal_act" as const,
    withdrawnBy,
    withdrawnAt: now,
    ...
    effectFraming: "hga_initiated_retraction" as const,
    hoemWithdrawalRecord,
    ...
    r98DistinctHgaWithdrawalAct: true as const,
    r99SharedPreconditionsPlusTriggers: true as const,
    r100NoWithdrawAfterRelianceCeased: true as const,
    r101NoSoleRtcGpraG11HrwmBasis: true as const,
    r102SingleBindingPostureChain: true as const,
    r103ConstitutionalBasisAndProvenance: true as const,
    r104EffectFromWithdrawnAtForward: true as const,
    r105HgaInitiatedRetractionCessation: true as const,
    r106AttributedBindingOnly: true as const,
    r107HoemWithdrawalOperativeRecord: true as const,
    r108NotAutomaticHslmPromotion: true as const,
    r109NoAdditionalCessationAfterCeased: true as const,
    r110InvalidAttemptsNonOperative: true as const,
    r111NotSuspensionRecallOrReentry: true as const,
    audit: Object.freeze({
      createdAt: now,
      createdBy: withdrawnBy,
      traceability: createGovernanceTraceability(["FI-DSN-STD-012-R40"]),
    }),
    traceability: GOVERNED_HANDOFF_WITHDRAWAL_TRACEABILITY,
