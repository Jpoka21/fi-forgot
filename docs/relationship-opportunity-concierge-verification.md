...
The frontend mirrors and maps the Opportunity without semantic renaming. With the Brain feature enabled, workspace and conversation components consume Opportunities directly and render actions only when `recommendation` is present. When the feature flag selects the legacy loader, both consumers retain the legacy recommendation fallback because that path intentionally has no Opportunity payload. The legacy recommendation adapter no longer maps priority into a field named confidence.

Attention selection, ordering, fatigue/exposure orchestration, caps, route, copy, links, feature flags, and rollback configuration were not changed. Primary visible Opportunity construction spans the union of the recommendation and insight caps, so every compatibility projection is derived from an available primary Opportunity even if those caps diverge.
