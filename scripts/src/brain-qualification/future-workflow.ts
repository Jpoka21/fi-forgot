...
if(fixtureHash!=="0e0df18b78c6e14c126d7f392eb9e101e4f9228f3f89173e646b2f1d0270fac8"||!existsSync(markerPath)||readFileSync(markerPath,"utf8")!=="fi-forgot-brain-qualification-owned-v1\n")throw new Error("FAIL_CLOSED: fixture drift or disposable marker missing");
...
if(action==="load"){
 ...
 try{const observed=(await adminPool.query("select current_database() as database,current_user as role,inet_server_addr()::text as address,inet_server_port() as port,current_setting('server_version_num') as version,current_setting('data_directory') as data_directory")).rows[0];if(observed?.database!=="postgres"||observed?.role!=="fi_forgot_brain_cluster_admin"||observed?.address!=="127.0.0.1"||Number(observed?.port)!==55432||Number(observed?.version)<160000||Number(observed?.version)>=170000||String(observed?.data_directory).replaceAll("\\","/").toLowerCase()!==markerPath.slice(0,markerPath.lastIndexOf("/")).toLowerCase())throw new Error("FAIL_CLOSED: admin-observed PostgreSQL identity mismatch before write");}finally{await adminPool.end();}
 ...
 try{const observed=(await pool.query("select current_database() as database,current_user as role,inet_server_addr()::text as address,inet_server_port() as port,current_setting('server_version_num') as version")).rows[0];if(observed?.database!==appConnection.database||observed?.role!==appConnection.user||observed?.address!=="127.0.0.1"||Number(observed?.port)!==55432||Number(observed?.version)<160000||Number(observed?.version)>=170000)throw new Error("FAIL_CLOSED: app-observed PostgreSQL identity mismatch before write");
 await db.transaction(async(tx:any)=>{await tx.insert(schema.usersTable).values(fixture.owners.map((x:any)=>({id:x.id,email:x.email,name:x.name}))).onConflictDoNothing();await tx.insert(schema.recipientsTable).values(fixture.recipients.map((x:any)=>({id:x.id,userId:x.userId,firstName:x.firstName,relationshipType:x.relationshipType,birthday:fixture.qualificationTiming.birthdayOffsetsDays[x.id]!=null?relativeDate(fixture.qualificationTiming.birthdayOffsetsDays[x.id]):x.birthday,anniversary:x.anniversary}))).onConflictDoNothing();await tx.insert(schema.personalRecipientsTable).values(fixture.recipients.map((x:any)=>({id:x.id,userId:x.userId,data:{id:x.id,name:x.firstName,relationship:x.relationshipType,birthday:fixture.qualificationTiming.birthdayOffsetsDays[x.id]!=null?relativeDate(fixture.qualificationTiming.birthdayOffsetsDays[x.id]):x.birthday,anniversaryDate:x.anniversary,active:true}}))).onConflictDoNothing();await tx.insert(schema.questionAnswersTable).values(fixture.questionAnswers.map((x:any)=>({...x,eventYear:Number(runDate.slice(0,4)),archivedAt:x.archivedAt?new Date(`${relativeDate(-30)}T00:00:00Z`):null}))).onConflictDoNothing();});console.log("approved synthetic production-schema fixture loaded");}finally{await pool.end();}
}else{
 ...
 if(!afterRestart){
   ...
   const workspace=await call('/v2/concierge','brain-qual-owner-a');
   const opportunity=workspace.opportunities?.find((o:any)=>o.timing?.temporal?.occurrenceCycleId),cycle=opportunity?.timing?.temporal?.occurrenceCycleId;
   if(!opportunity?.id||!opportunity?.recipient?.id||!cycle)throw new Error('production Opportunity lacks mutation identity');
   const recipientId=opportunity.recipient.id,opportunityId=opportunity.id;
   const feedbackMutation=await call('/v2/concierge/opportunity-feedback','brain-qual-owner-a',{method:'POST',body:JSON.stringify({recipientId,opportunityId,occurrenceCycleId:cycle,type:'helpful',expectedVersion:0,idempotencyKey:'brain-qual-feedback-helpful-v1'})});
   if(!feedbackMutation.feedback?.id||feedbackMutation.feedback.active!==true)throw new Error('feedback receipt/active state missing');
   const actionMutation=await call('/v2/concierge/opportunity-follow-through','brain-qual-owner-a',{method:'POST',body:JSON.stringify({recipientId,opportunityId,occurrenceCycleId:cycle,dimension:'action',value:'user_reported_completed',expectedVersion:0,idempotencyKey:'brain-qual-follow-action-v1'})});
   const event=actionMutation.followThrough;
   if(!event?.id)throw new Error('follow-through receipt missing');
   const outcomeMutation=await call('/v2/concierge/opportunity-follow-through','brain-qual-owner-a',{method:'POST',body:JSON.stringify({recipientId,followThroughEventId:event.id,dimension:'outcome',value:'went_well',expectedVersion:0,idempotencyKey:'brain-qual-follow-outcome-v1'})});
   if(!outcomeMutation.followThrough?.id||outcomeMutation.followThrough.dimension!=='outcome'||outcomeMutation.followThrough.active!==true)throw new Error('separate outcome receipt/active state missing');
 ...
 for(const owner of fixture.owners){
   ...
   if(!workspace.opportunities?.length||workspace.recommendations?.length>3)throw new Error('empty Opportunity projection or maximum-three violation');
   ...
   snapshots.push({owner:owner.id,ids,timelines,feedback,followThrough,opportunityIds:workspace.opportunities.map((o:any)=>o.id).sort()});
 }
 const snapshot={fixtureHash,syntheticRunDate:runDate,snapshots};
