...
const future=bytes("scripts/src/brain-qualification/future-workflow.ts").toString();for(const value of["fixtureHash","markerPath","parseQualificationAdminDatabaseUrl","inet_server_addr","data_directory","pool.end","--after-restart","qualifyUnderstanding","cross-owner feedback","qualifyOpportunitySemantics","opportunitySemantics","assertTemporalExamples","semanticCoverage"])if(!future.includes(value))throw new Error(`future workflow guard/lifecycle drift: ${value}`);
