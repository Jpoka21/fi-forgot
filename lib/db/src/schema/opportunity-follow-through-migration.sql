...
CREATE TABLE IF NOT EXISTS opportunity_follow_through_events (
  ...
  source_type text NOT NULL CONSTRAINT opportunity_follow_through_source_type_check CHECK (source_type = 'brain_execution'), source_id text NOT NULL,
  dimension text NOT NULL CONSTRAINT opportunity_follow_through_dimension_check CHECK (dimension IN ('action','outcome')),
  value text NOT NULL, action text NOT NULL CONSTRAINT opportunity_follow_through_action_check CHECK (action IN ('set','withdraw')),
  provenance text NOT NULL CONSTRAINT opportunity_follow_through_provenance_check CHECK (provenance = 'explicit_owner_report'),
  verification text NOT NULL CONSTRAINT opportunity_follow_through_verification_check CHECK (verification = 'user_reported'),
