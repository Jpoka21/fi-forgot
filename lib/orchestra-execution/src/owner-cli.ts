...
import {
  ...
  GITHUB_CONTROL_DEFAULT_PROVIDER,
...
function buildStatus(args: ParsedArgs): unknown {
  ...
  return {
    ...
    githubControlWatcher: {
      ...
      defaultProvider: GITHUB_CONTROL_DEFAULT_PROVIDER,
      startCommand: "pnpm --filter @workspace/orchestra-execution start:github-control",
    },
...
function printStatus(payload: any, io: OwnerCliIo): void {
  io.out([
    ...
    `GitHub control watcher: ${payload.githubControlWatcher.available ? "available" : "unavailable"} (${payload.githubControlWatcher.repository}, provider ${payload.githubControlWatcher.defaultProvider})`,
