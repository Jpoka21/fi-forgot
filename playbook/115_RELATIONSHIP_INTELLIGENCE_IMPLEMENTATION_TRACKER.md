...
Current-source reconciliation also detects answers changed by existing non-timeline producers. It appends only the newly captured current truth, preserves prior revisions, advances the head with compare-and-swap, and invalidates interpretations pinned to the displaced version. Repeated unchanged reads are inert. Grouped briefing presentation and mutation restrictions remain intact, with member-source histories exposed as read-only observation history.
# Evolving understanding production slice

Implemented the bounded `explicit-recipient-communication-preference` Brain hypothesis family (policy v3): conservative recipient-scoped grammar and abstention, immutable scoped versions and heads, exact evidence and interpretation-revision provenance, deduplicated support, conflict/unknown/retirement/substantively-new-evidence revival semantics, version-bound endorsement, persistent disagreement/withdrawal suppression, non-confirming reversal, authenticated timeline transport, eligible recipient Brain-context projection, and production-transaction recomputation. Hypotheses remain uncertain, confidence-null, distinct from facts/interpretations/recommendations, and cannot force action. See `docs/evolving-understanding-verification.md`.

Deferred: schema application, deployment, live PostgreSQL concurrency qualification, and authenticated owner-visible workflow qualification.
## Opportunity temporal lifecycle (implementation slice)

Implemented a server-owned, evidence-aware temporal contract for retained Relationship Opportunities. Birthday and anniversary signals receive strict annual-cycle evaluation; explicit one-time callers preserve occurrence identity; malformed, absent, and unsupported timing remains unknown. The contract separates evidence, confidence, importance/priority, timing, qualitative decay, eligibility, restraint, and recommendation existence. Known ineligible timing remains retained but silent, with no lower-ranked backfill and the established maximum-three presentation cap preserved.

Added append-only evaluation history semantics, an unapplied database schema, and production-builder load/append wiring through an owner-scoped idempotent repository. Unknown withdrawal, archive, invalidation, and invalid replacement transitions append without destroying the last known occurrence. Live schema deployment, live database qualification, and live authenticated workflow verification remain deferred. Orchestra2 owns checkpointing and gated automatic publication after passing verification and independent acceptance. Verification details and exact limitations are recorded in `docs/opportunity-temporal-verification.md`.

Correction completed: the production repository enumerates retained Opportunities by owner and authorized recipient, reloads identities missing from current decisions, stores shallow Opportunity plus immutable evidence snapshots, and uses a collision-safe scoped row identity with a separately retained semantic change key. Unchanged evaluations are inert; displaced occurrences, withdrawal, archive, invalidation, and changed/invalid evidence append non-destructively. History listing/load failures cannot establish a false baseline, append failures are explicitly non-durable and silent, and temporally silent items are excluded from delivered fatigue exposure. The production integration covers the genuine annual-source-to-fixed-one-time displacement path and actual-adapter owner/recipient isolation. The current outer-controller verification receipt records the authoritative scoped build outcomes; worker sandbox esbuild restrictions are not live qualification or a waiver of those checks.
# Opportunity feedback slice (implementation complete; rollout deferred)

- Added nine-type explicit, reversible Opportunity preference history with occurrence-default and explicitly labeled recipient-family scope.
- Added unapplied append-only PostgreSQL event and request-receipt schema plus production repository wiring with fingerprinted idempotency, inert equal repeats, optimistic version conflicts, supersession, and historical withdrawal.
