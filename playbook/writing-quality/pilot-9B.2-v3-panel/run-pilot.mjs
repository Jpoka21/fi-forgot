/**
 * Sprint 9B.2 attempt 3 — targeted seven-scenario panel runner.
 ...
 *   - Writes only under pilot-9B.2-v3-panel/
...
function collectProvenance() {
  return {
    ...
    productionImplementationNote:
      "Sprint 9B.2 attempt 3 (professional Thank You body/sign-off boundary). Harness commits after this SHA are evaluation assets only.",
...
function toMd(corpus) {
  const lines = [
    `# Sprint 9B.2 Attempt 3 — Targeted Panel Corpus`,
    ``,
...
function runSelfCheck() {
  ...
  expect("out dir is pilot-9B.2-v3-panel", path.basename(__dirname) === "pilot-9B.2-v3-panel");
  expect(
  ...
  expect(
    "does not write into pilot-9B.2-v2-panel",
    path.resolve(outDir) !== path.resolve(frozen9B2V2PanelDir),
  );
...
async function main() {
  ...
  if (!base) {
    const blocker =
      "BLOCKER: No reachable server for POST /api/v2/generate-card on PILOT_BASE_URL or ports 3000/5000/8080. Cannot generate Sprint 9B.2 attempt 3 panel.";
    console.error(blocker);
    process.exitCode = 2;
    const corpus = finalizeCorpusStatus({
      corpusId: `${CORPUS_PREFIX}-${date}`,
      title: "Sprint 9B.2 Attempt 3 Targeted Panel",
      ...
      writingContractNote:
        "Evaluates production writing at 38e1d74fd3a2825703b78d9638877bf8743fd395 (Sprint 9B.2 attempt 3 body/sign-off boundary). Targeted seven-scenario panel only.",
  ...
  const corpus = finalizeCorpusStatus({
    ...
    writingContractNote:
      "Evaluates production writing at 38e1d74fd3a2825703b78d9638877bf8743fd395 (Sprint 9B.2 attempt 3 body/sign-off boundary). Targeted seven-scenario panel only — not full G01–G20.",
