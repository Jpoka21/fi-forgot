/**
 * Sprint 9B.2 evaluation harness — full 20-scenario corpus runner.
 * Evaluation-only. Does not modify production prompts or frozen Sprint 9B.1 / 9A.2 artifacts.
 *
 * Usage:
 ...
 *
 * Semantics (Sprint 9B.2 verification):
 ...
 *   - Writes only under pilot-9B.2-v1/ — never touches pilot-9B.1-v2/ or pilot-9A.2/
 *   - status "complete" only when all 20 scenarios have attempts===1 and one nonempty card
 *
 * Provenance:
 *   productionImplementationCommit is pinned to the Sprint 9B.2 production tip (0d47a18…).
...
const CORPUS_PREFIX = "pilot-9B.2-v1";

/**
 * Production writing implementation under evaluation for Sprint 9B.2.
...
function collectProvenance() {
  return {
    ...
    productionImplementationNote:
      "Sprint 9B.2 temporary evaluation tip (professional Thank You anti-gratitude-stack). Harness commits after this SHA evaluate this implementation; they are not a new writing tip unless production changes land.",
...
function toMd(corpus) {
  const lines = [
    `# Sprint 9B.2 Evaluation Corpus`,
    ``,
    corpus.notForScoring
      ? `> **NOT FOR SCORING** — status \`${corpus.status}\`. Do not treat as a completed Sprint 9B.2 evaluation.`
    ...
    `- frozen Sprint 9B.1 Framework V2 at pilot-9B.1-v2/ not overwritten`,
    ``,
  ];
  if (corpus.blocker) {
...
function runSelfCheck() {
  ...
  expect("out dir is pilot-9B.2-v1", path.basename(__dirname) === "pilot-9B.2-v1");
...
async function main() {
  ...
  if (!base) {
    const blocker =
      "BLOCKER: No reachable server for POST /api/v2/generate-card on PILOT_BASE_URL or ports 3000/5000/8080. Cannot generate Sprint 9B.2 corpus.";
    console.error(blocker);
    process.exitCode = 2;
    const corpus = finalizeCorpusStatus({
      corpusId: `${CORPUS_PREFIX}-${date}`,
      title: "Sprint 9B.2 Evaluation Corpus",
      ...
      writingContractNote:
        "Evaluates production writing at 0d47a18163c52a5c9f8773c86ccc3414c24b2e47 (Sprint 9B.2 professional Thank You anti-gratitude-stack). Harness commits after that SHA are evaluation assets only.",
